...
Container ports, `/api` proxy, and healthchecks: `docs/deployment/ecs-containers.md`. Coding invariants when editing Dockerfiles: `CLAUDE.md` (ECS container invariants section).
...
| `.claude/skills/update-ecr-images/scripts/push.sh` | Full login + push + ECS redeploy |
...
```bash
.claude/skills/update-ecr-images/scripts/push.sh           # both
