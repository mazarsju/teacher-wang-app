...
- A **data catalog** (a list of translated records, not one component's copy) gets its own data-namespace JSON keyed by a stable per-item slug instead of a component name — e.g. `challenge.json` for `frontend/src/data/challenges.ts`'s `title`/`description`/`character.name`/`character.description`/`tasks[].label` — rendered via a `get<Catalog>(t)` function reading a template array that keeps non-translatable fields (ids, `chineseName`, `avatarVariant`) in TS. Follow this same shape for any other catalog. `frontend/src/data/chatCharacters.ts` still hardcodes English on purpose — see the ADR's "Out of scope" section before changing that judgment call.
