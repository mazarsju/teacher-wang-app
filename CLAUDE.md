...
8. **Did you edit a `.claude/skills/<name>/SKILL.md`?** → Check whether `.cursor/skills/<name>/SKILL.md` exists; if it does, apply the same content change there too (they are separate copies — Claude Code reads `.claude/skills`, Cursor reads `.cursor/skills` — with no automatic sync, so a one-sided edit is itself a doc-hygiene bug, same as item 7's rules/CLAUDE.md pair).
9. **Did you make a judgment call, discover a non-obvious constraint, or hit a tooling quirk mid-task that isn't written down anywhere** (e.g. a CLI flag that only works in one particular form)? → Capture it in the relevant doc/rule section rather than leaving it to be rediscovered next session. If unsure where it belongs, say so in the summary and ask rather than silently dropping it.
...
- Auth is global via `before_request` in `backend/utils/auth/user_context.py`. Do not add per-route auth decorators. Public routes: `OPTIONS`, `/health`, and the entries in `PUBLIC_PATHS` (pre-session routes like password reset) only.
- Every new route must require authentication by default. Never add a path to `PUBLIC_PATHS` unless it is a pre-session endpoint the user has no token for yet (login, signup, password reset, health check); anything reachable by an authenticated user (including read-only/list endpoints) must stay off that list. When adding a route, confirm it is NOT in `PUBLIC_PATHS` and add a `backend/tests/test_auth.py`-style assertion that it 401s without a token.
...
## LLM/AI call token quota

Applies to: `backend/**/*.py`.

Rationale: `docs/adr/plan-management.md` (the quota mechanism), `docs/adr/voice-interaction.md` (a worked non-chat example).

Any new feature that talks to an LLM or an LLM-adjacent provider (chat completion, TTS, STT, embeddings, image generation, ...) must both **gate** on the caller's quota before spending money and **charge** actual usage against it after — never add a call that does one but not the other, and never call `get_llm()`/`get_openai_client()` directly outside the two patterns below. Every plan is gated this way, not just free — pro gets a higher `PRO_PLAN_TOKEN_GRANT` budget instead of a skip.

- **Chat-completion (langchain `messages` list) calls**: go through `_invoke_llm` in `backend/utils/aiChat/chat_service.py` — never call `get_llm().invoke(...)` yourself. It calls `assert_plan_has_tokens(user)` (`backend/utils/database/settings.py`) before invoking, and `deduct_available_token` after, keyed off `current_user()` by default.
- **Any other provider call** (TTS, STT, or a future one that isn't a langchain messages list): call `assert_plan_has_tokens(user)` yourself before making the call, and after a successful call record + deduct the usage (`record_token_usage` + `deduct_available_token`, or a shared per-file helper like `_charge_token_usage` in `backend/routes/chat.py`). When the provider doesn't return real token usage (TTS input text, an STT transcript), estimate it with `estimate_text_tokens()` (`backend/utils/aiChat/token_usage.py`) rather than skipping accounting — see `POST /chat/tts`/`POST /chat/stt` in `backend/routes/chat.py` for the full pattern.
- **A call with no Flask request context** (a background thread, a cron/batch job) can't resolve `current_user()`. If the call is on behalf of one specific learner, resolve that user's row yourself (e.g. `User.query.filter_by(shortid=user_id).first()`) and pass it explicitly (`_invoke_llm(messages, user=user_row)`) — see `_summarize_and_store` in `backend/utils/aiChat/conversation_summary.py`. A true system-wide job with no single owning user (e.g. weekly article generation) is deliberately exempt — `_invoke_llm` treats "no resolvable user" as ungated — but that exemption is only for jobs that aren't billed to one learner; don't reach for it to skip gating on a per-user feature.
- A quota-exhausted call returns `400` with a plan-appropriate message — `FREE_PLAN_TOKEN_EXHAUSTED_MESSAGE` or `PRO_PLAN_TOKEN_EXHAUSTED_MESSAGE` (`backend/utils/database/settings.py`, picked by `assert_plan_has_tokens` itself) — reuse those, don't invent a new one.
- Tests: a new gated endpoint needs a test asserting the `400`-on-exhausted-quota path (with `assert_plan_has_tokens` mocked to raise) and a test asserting a successful call records/deducts tokens — see `backend/tests/test_chat_tts_route.py`/`test_chat_stt_route.py`.
...
- One JSON resource file per feature namespace under `frontend/src/locales/en/<namespace>.json` (`common`, `home`, `chat`, `challenge`, `knowledge-base`, `grammar`, `writing`, `preferences`, `admin`, `auth`). A component used from a single feature page keeps its strings in that feature's namespace; a component reachable from more than one feature (grep its import sites to check) goes in `common` instead of duplicating its copy across namespaces.
- A **data catalog** (a list of translated records, not one component's copy) gets its own data-namespace JSON keyed by a stable per-item slug instead of a component name — e.g. `challenge.json` for `frontend/src/data/challenges.ts`'s `title`/`description`/`character.name`/`character.description`/`tasks[].label` — rendered via a `get<Catalog>(t)` function reading a template array that keeps non-translatable fields (ids, `chineseName`, `avatarVariant`) in TS. Follow this same shape for any other catalog. `frontend/src/data/chatCharacters.ts`'s `description` field is translated this way (`getTeacherWang`/`getXiaoMing`/`getChatCharacters` reading `chat.json`'s `chatCharacters` key); its `name`/`chineseName`/`avatarVariant` stay hardcoded on purpose — see the ADR's "Out of scope" section before changing that judgment call.
