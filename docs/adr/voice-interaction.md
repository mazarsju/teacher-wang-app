...
5. Strips what's left down to Chinese characters, digits, and full-width Chinese punctuation (`_chinese_digits_and_punctuation_only`, a `[一-鿿0-9，。！？：；、“”‘’（）《》…—～]+` regex extraction) before responding — Whisper also occasionally emits stray Latin words/punctuation even for real speech, and this app only ever wants Hanzi (plus numbers, e.g. an age, a room number, a date, and the sentence punctuation Whisper genuinely transcribes, e.g. "我的家有5口。") in the message input. **Half-width** punctuation (`,`/`.`/`!`/`?`/...) is deliberately excluded from that set — in practice it only ever shows up glued to stray Latin text (Whisper writes real Mandarin punctuation in its full-width form), so keeping it would let exactly the kind of noise this filter exists to remove back in. If nothing survives either filter (silence, background noise, non-Mandarin speech), the response is `{ "text": "" }`; the frontend treats an empty string as "no speech detected" and shows an error instead of clearing whatever the learner had already typed.
