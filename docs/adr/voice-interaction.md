...
- In `listening_first` mode: the reply text renders CSS-blurred behind an eye ("reveal") button; the listen button stays visible next to it so both controls are reachable while the text is masked. Audio for a fresh reply autoplays as soon as it's fetched. Reveal is a per-message toggle (`toggleMessageReveal`, backed by a `Set<number>` of revealed message indices) — revealing swaps the eye button for a "blur again" one next to the listen button, so a learner can re-mask a message after peeking at it; both the mask and the audio cache reset when the conversation/character changes.
...
2. Calls `get_openai_client().audio.transcriptions.create(model="whisper-1", file=(filename, bytes, mimetype), language="zh", response_format="verbose_json", prompt=expected_text)`. The `language="zh"` hint is hardcoded, not derived from the learner's app locale — every character in this app is a Mandarin-speaking role regardless of the UI language the learner reads in. `verbose_json` (instead of the default plain-text format) is what makes per-segment confidence available for step 4. `expected_text` is an optional multipart form field, read from `request.form.get("expected_text")` and truncated to 200 chars defensively — when the caller knows in advance what the learner was asked to say (shadowing: the sentence being repeated), sending it as Whisper's `prompt` biases decoding toward that text/vocabulary, which helps disambiguate near-homophone sentences that otherwise transcribe as an unrelated-but-similar-sounding phrase (e.g. "坐几号车" mis-heard as "做鸡好吃"). Chat's free-form recording has no expected answer to hint with, so `transcribeChatAudio` never sends this field and `prompt` is just an empty string (a no-op for Whisper) — this is the one place shadowing's STT call shape differs from chat's; see [listening-practice.md](./listening-practice.md) decision 4.
3. On success, charges `estimate_text_tokens(transcript.text)` as `output_tokens` (the learner's spoken words, transcribed, are the "generated" side of this call, mirroring how a chat reply's tokens are `output_tokens`) via the same `_charge_token_usage` helper as `/chat/tts`. Charging uses the raw Whisper transcript, before either filter below is applied, since that's the actual cost incurred.
4. Drops hallucinated segments (`_drop_silent_segments`) before the Chinese/digit filter runs. Whisper is known to hallucinate memorized training-data phrases — most visibly subtitle-credit boilerplate like "由社群提供的字幕" ("subtitles provided by the community") — when the audio has no real speech (silence, a misclick, background noise) instead of returning empty text; because the hallucinated phrase is itself valid Chinese, a plain character-range filter can't catch it. `verbose_json` returns each segment's `no_speech_prob` and `avg_logprob`; a segment is dropped only when **both** are bad (`no_speech_prob > 0.6` and `avg_logprob < -1.0` — either alone is too weak a signal: a quiet-but-real utterance can have a high `no_speech_prob` yet still be transcribed confidently, and shouldn't be discarded just for that). Surviving segments' text is concatenated back together.
5. Strips what's left down to Chinese characters, digits, and punctuation (`_chinese_digits_and_punctuation_only`) before responding — Whisper also occasionally emits stray Latin words/punctuation even for real speech, and this app only ever wants Hanzi (plus numbers, e.g. an age, a room number, a date, and the sentence punctuation Whisper genuinely transcribes, e.g. "我的家有5口。") in the message input. This isn't a single regex whitelist: Whisper doesn't consistently use full-width punctuation for Mandarin speech (`，。！？…`) — it sometimes emits plain ASCII `,`/`.`/`!`/`?` too — so both forms are accepted, but a matched run of characters is kept only if it contains at least one Hanzi or digit; a run of pure punctuation with no Chinese/digit anywhere in it (e.g. the comma orphaned once a hallucinated "Hello," has its Latin letters stripped) is discarded. That's what still lets punctuation glued to stray hallucinated Latin text get dropped, while punctuation genuinely attached to real Chinese/digit speech — regardless of which width Whisper happened to use — survives. If nothing survives either filter (silence, background noise, non-Mandarin speech), the response is `{ "text": "" }`; the frontend treats an empty string as "no speech detected" and shows an error instead of clearing whatever the learner had already typed.
