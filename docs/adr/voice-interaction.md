...
5. Strips what's left down to Chinese characters and digits (`_chinese_and_digits_only`, a `[一-鿿0-9]+` regex extraction) before responding — Whisper also occasionally emits stray Latin words/punctuation even for real speech, and this app only ever wants Hanzi (plus numbers, e.g. an age, a room number, a date) in the message input. If nothing survives either filter (silence, background noise, non-Mandarin speech), the response is `{ "text": "" }`; the frontend treats an empty string as "no speech detected" and shows an error instead of clearing whatever the learner had already typed.
