...
`ListeningPage` shows the catalog as an active mosaic (topics with `status !== "DONE"`) sorted by `vocabulary_score + grammar_score` (an "overall score" that exists only in the frontend, never persisted, via the same 4-tier `scoreTier` — excellent/good/fair/poor — `ListeningScoreModal` uses for its fit message), plus a native `<details>` "Completed practices (N)" section below it — collapsed by default, expanding to the same tile mosaic for every `status === "DONE"` topic, the same collapsible pattern `GrammarPage` already uses for its per-level sections. Each tile's *background* is a soft top-to-bottom gradient from that tier's color (blue/green/yellow/red) into the tile's normal surface color, and its score button shows a custom face icon (`VeryHappyFaceIcon`/`HappyFaceIcon`/`NeutralFaceIcon`/`UnhappyFaceIcon`, `components/icons.tsx`) — a filled circle plus white eyes/mouth, same style as `CheckIcon`/`IncorrectIcon` — colored via `currentColor` to the same base color as its tile's gradient, rather than a plain color swatch or a platform emoji (whose rendering and color aren't controllable), so the fit reads at a glance without opening the modal — the modal itself (`ListeningScoreModal.tsx`) bolds and colors each percentage via a `<Trans>`-wrapped `<strong>` (own `percentTier` thresholds: 100% blue, ≥80% green, ≥60% amber, else red — accessible text-color variants, not the tile's raw fill colors) and bolds/colors its closing fit sentence in the same `scoreTier` color as the tile it was opened from; title and a `type` badge and a `topic` badge sit on top — the two badges each colored by hashing their untranslated value into a fixed palette so the same value always gets the same color across languages and independently of the tile's tier color. Clicking a tile opens `ListeningPracticeDetailPage`, which has five sections: **Listen** (`AudioPlayer` on the full `audio.mp3`, with ±5s skip); **Questions** (`ListeningExercises` — see below); **Shadowing** (one `ShadowingSentence` row per breakdown sentence — its own `AudioPlayer` on that sentence's clip, the Mandarin text blurred behind an eye toggle, and a type-or-record input checked against the sentence with a lenient punctuation/whitespace-insensitive comparison, `matchesSentence.ts`); **Full text** (the whole `text.txt` blurred behind an eye toggle, with a "Show translation" button revealing the sentences' `translation` fields joined by newline — reusing the same per-sentence data the detail fetch already returned, rather than a second backend field); and **Completion** (see below), always the last section on the page. The record-or-type input's speech-to-text reuses `POST /chat/stt` verbatim (same Whisper call, same free-plan token gating) — no new STT endpoint was added for this. Because a plain `<audio src>` can't carry the app's `Authorization` header, `AudioPlayer` fetches each clip's bytes via `apiFetch` and plays them from a `URL.createObjectURL` blob (lazily, on first play, and cached for replay) rather than pointing at the route URL directly — the same blob-from-fetch pattern `ChatModal` already uses for TTS playback.

**Questions** (`ListeningExercises.tsx`) reads the topic's `exercises` (multiple-choice only — `answer` sent to the client, graded there, same tradeoff `GrammarExercises` already makes for its own exercises) and, unlike `GrammarExercises`'s one-at-a-time flow, renders every question on the page at once — a deliberate presentation difference, not a missing feature: comprehension questions here are short and few, so there's no pacing benefit to hiding them behind a "next" click. A single "Verify" button (disabled until every question has an answer) computes a percentage, shows each choice as correct/incorrect in place, and — same 80% threshold and `ChallengeConfetti` component `GrammarExercises` already uses for its own passing score — fires the confetti and shows a pass/fail message. This is purely a local, ephemeral comprehension check: the score never reaches the backend and never changes `status` (changing an answer after verifying just clears the shown result, keeping the selections, until Verify is pressed again) — whether the topic counts as done is a separate, deliberate decision the learner makes in the **Completion** section.

**Completion**, the last section on `ListeningPracticeDetailPage`, asks "Do you consider this listening practice completed?" with Yes/No buttons that call `POST /listening-practices/<id>/complete` with `{"completed": true|false}` (`backend/routes/complete_listening_practice.py`, no score threshold — see [schema tenancy](../architecture/schema-tenancy.md)); the response's `status` is merged back into the page's local state (so a "Marked as completed."/"Marked as not completed yet." line reflects it immediately without a refetch) and also drives which of `ListeningPage`'s two sections the topic sorts into next time the learner sees the catalog. This replaced an earlier version of this endpoint that inferred `DONE`/`WIP` from the Questions score at the same 80% threshold `complete_grammar_point.py` still uses — the two now differ in whether completion is judged or self-reported for a topic that is fundamentally "have you internalized this", not "did you get the questions right".
