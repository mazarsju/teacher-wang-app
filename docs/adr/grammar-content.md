...
The content pipeline described here (repo → S3 → `grammar_points` / `grammar_prerequisites`) is implemented. The frontend has a Grammar tab listing grammar points, loaded in stages rather than one big call, and dispatched once at login (`frontend/src/store/thunks/loadGrammarData.ts`, fired from `App.tsx` *before* `syncAppData` — not from the Grammar tab itself, so the data is already loading, or loaded, by the time the learner opens it): `GET /hsk-level-light` (the learner's persisted HSK level, no character/vocabulary scan) is the very first request of the login sequence, ahead of `syncAppData`'s own calls and `GET /auth/me`, and once its answer is in, `GET /grammar-points-light` returns every point's catalog-only fields (`hsk_level`, `index` — the folder's numeric prefix, stored on `grammar_points.index` at reload time rather than parsed from `s3_key` per request — `title`, translated by querying `grammar_points_translate` for the caller's language instead of a live S3 read, ordered by HSK level then that index, e.g. `hsk1/01-…` before `hsk1/02-…`), letting the Grammar tab render the table immediately; `GET /grammar-points/<hsk_level>` then fetches each visible level's `prerequisites` and the current user's `status`/`score`/`usage_count` from `user_grammar_progress`, one call per level (run in parallel with each other), filling in the status/score columns (a spinner shows until that level's call resolves) — each level's response is merged into the store without disturbing any other level's already-loaded data. `loadGrammarData` runs at most once per session: a manual "Synchro" re-runs `syncAppData` but not this, so `hsk-level-light` and the grammar endpoints are never re-called after login. The Grammar tab shows points up to one HSK level above the learner's achieved level (the level they are aiming for; capped at the catalog max) and a detail view (`GET /grammar-points/<id>`, `backend/utils/grammar/grammar_content_loader.py:fetch_grammar_content`) that reads `explanation.md`/`exercises.json` from S3 at the point's `s3_key` and renders the Explanation tab as Markdown (reusing the chat agents' `renderFormattedText`, `frontend/src/utils/formatMarkdownText.tsx`). The Explanation tab ends with an "Ask more information to Teacher Wang" button that opens an ephemeral, lesson-scoped chat (see the Multi-Agent Chat ADR's "Ephemeral chat" section). The Exercises tab (`frontend/src/components/GrammarExercises.tsx`) runs the `multiple_choice`, `sentence_reordering`, `transform`, and `translation` exercises one at a time with deterministic grading (exact match, trailing punctuation ignored) and shows a final score. `translation` additionally falls back to an AI check when the exact match fails — see decision 5 below, now implemented. Finishing a quiz calls `POST /grammar-points/<id>/complete`, which sets `status` to `DONE` (score ≥ 80%) or `WIP` (below 80%), plus `score` (rounded percentage) and `last_practiced_at`, on `user_grammar_progress`; `GET /grammar-points/<hsk_level>` returns that `score` alongside `status` for the list view. Grammar points also loop back into chat: after any Teacher Wang chat reply where the grammar correction agent found no mistake (`correction.severity === "none"`), the frontend calls `POST /grammar-points/check` (`pro` plan only) with the learner's last message; an LLM agent (`check_grammar_usage`, `backend/utils/aiChat/chat_service.py`) checks it against the learner's `DONE` grammar points and increments `usage_in_real_life` for each one it finds used. After 3 real-life uses a point's `status` flips to `MASTERED` (shown with a blue badge/star icon and blue score on the Grammar tab, `frontend/src/pages/GrammarPage.tsx`), and the frontend shows a `GrammarMasteryModal` with the completion confetti listing the newly mastered lesson titles. AI-powered practice and AI-assisted translation validation are not yet built (README roadmap §10).
...
This is a shared table (no `user_id`), like `grammar_points` itself, since it's curriculum catalog data rather than learner state — `overview.yaml` plays the same role for a writing topic that `grammar.yaml` plays for a grammar point, and reuses the loader's existing S3-listing/local-checkout code path rather than a second implementation. `GET /writing-practices` returns every `writing_practice` row with the current user's `status` (replacing what used to be a hardcoded frontend array, and split out from `GET /grammar-points` into its own endpoint alongside the grammar-points light/per-level split above), and `GET /writing-practice/<topic_id>` (`backend/routes/writing_practice.py`) fetches one topic's row plus its sibling `context.md` — the same explanation-content-lives-in-S3 split `fetch_grammar_content` uses for `explanation.md` — alongside the learner's draft/archive for that topic. See the [writing practice ADR](writing-practice.md), decision 1.

A `listening_practice/<hskN>/<name>/overview.yaml` + sibling `text.txt` layout lives in the same bucket, one level down from `grammar/` and `writing_practice/`, populating a `listening_practice` table (`backend/utils/listening/listening_content_loader.py`) via its own `reload_listening_content()`/`POST /admin/listening/reload` — a separate clear-and-repopulate pass from `reload_grammar_content()`, since listening topics aren't part of the grammar/writing curriculum graph. Full decision record, catalog/progress/exercise/shadowing flow, and plan-gating notes: see the [listening practice ADR](listening-practice.md). Full schema: [schema tenancy](../architecture/schema-tenancy.md).
