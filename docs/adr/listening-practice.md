...
Each `breakdown.json` sentence also carries a `speaker` field (the speaking character's Chinese name, e.g. `"大卫"` — empty string if absent). `fetch_listening_breakdown` passes it through as-is; a chunk has no `speaker` of its own and always inherits its parent sentence's. `ListeningPracticeDetailPage` resolves it to `man.png`/`woman.png` by comparing it against the topic's own `man_name`/`woman_name` (see above) and shows the matching picture before that sentence's/chunk's shadowing audio player — the same client-side name match, just applied per-sentence instead of once for the whole topic.
