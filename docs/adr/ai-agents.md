...
The Explanation tab's "Ask more information to Teacher Wang" button (`frontend/src/pages/GrammarPointDetailPage.tsx`) is a second ephemeral use case, this time topic-scoped: `POST /chat` also accepts a `context: string` field (ephemeral only), which `_handle_ephemeral_chat` forwards to `generate_chat_reply` as `topic_context`. When set, `generate_chat_reply` appends an instruction block plus the raw lesson content (title + `explanation.md`) to Teacher Wang's system prompt, telling it to stay scoped to that lesson (small digressions only if they help explain it) — the same mechanism `summary_context` uses to extend the system prompt, just for a different purpose. `ChatModal`'s `topicContext` prop threads this through every `sendChatMessage` call for the conversation's lifetime, so follow-up turns stay scoped too.

The modal opens with a scripted Teacher Wang greeting ("Ask me what you did not understand...") the learner never actually receives from the LLM. That message is flagged `isDisplayOnly: true` (`types/chat.ts`) — shown as a normal assistant bubble, but filtered out of `messagesToSend` before every `sendChatMessage` call, so it's never part of what's sent to the API or, had persistence been enabled, ever written to a log. `isDisplayOnly` and `isContext` are orthogonal: `isContext` changes how a message renders but it's still sent to the API (the "More explanation" question must reach the LLM); `isDisplayOnly` changes what's sent but not how it renders (the greeting must look like a normal reply).
...
2. Asks the LLM to **merge** those messages into the existing stored memory (not summarize them alone), using one of two schemas/prompts: `TEACHER_WANG_SUMMARY_SYSTEM_PROMPT` (teaching context: topics, grammar points, learner strengths/difficulties, vocabulary, unresolved questions) for `teacher-wang`, or `GENERIC_SUMMARY_SYSTEM_PROMPT` (situation, events, participants, open threads) for every other character. This call goes through `_invoke_llm` like any other chat LLM call, gated/charged against the learner's free-plan `available_token` budget — but since this background thread has no Flask request context, it can't resolve `current_user()`, so it looks up the `User` row from `user_id` itself and passes it in explicitly (see [plan-management.md](./plan-management.md)). A learner whose quota is already exhausted simply gets no summary for that turn (the job logs and swallows the resulting error, same as any other summarization failure) rather than a user-visible error, since this is a best-effort background feature.
