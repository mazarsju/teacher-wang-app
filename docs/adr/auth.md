...
* **Admin account deletion:** `DELETE /admin/users/<id>` (`backend/routes/delete_user.py`, gated on `ADMIN_EMAIL` like the other `/admin/users` routes) wipes the target's rows across every private table, deletes their conversation-log prefix, then calls `AdminDeleteUser` on Cognito (`backend/cognito_admin.py`). Unlike `cognito_public.py`, this is an IAM-signed admin action via boto3 (the ECS task role needs `cognito-idp:AdminDeleteUser` on the User Pool), not an unauthenticated app-client call.
