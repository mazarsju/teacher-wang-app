...
Writing topics appear inline in the Grammar tab's lesson list (`frontend/src/pages/GrammarPage.tsx`), not as a separate top-level nav item: `GET /writing-practices` returns `writing_practices` (`{id, title, after_grammar_point, status}[]`, read from the `writing_practice` Postgres table — see decision 1), and each entry is inserted as a "Practice: `<title>`" row immediately after the grammar lesson whose id matches `after_grammar_point`, with a pen icon in place of the usual lesson number and the same `StatusBadge` grammar-point rows use. Clicking it sets `selectedWritingTopicId` and renders `<WritingPracticeDetailPage topicId onBack>`.
...
`writing_progress` (`user_id`, `writing_topic` FK → `writing_practice.id`, `status`) is the Postgres side of a topic's progress: `WIP`/`DONE`, read back as `writing_practices[].status` above. See decision 7 for how it's kept up to date.
...
`GrammarPage.tsx` no longer imports a static array: `GET /writing-practices` returns a `WritingTopic[]` of `{id, title, after_grammar_point, status}` (`status` is the learner's own `writing_progress` row, decision 7), fetched once at login by `loadGrammarData` alongside (not as part of) the grammar points' own light/per-level calls (see the [grammar content ADR](grammar-content.md), decision 7) — not on Grammar tab mount — and `WritingPracticeDetailPage.tsx` fetches its own `title`/`context` (plus `draft`/`archive` — decision 10) from `GET /writing-practice/<topic_id>` on mount instead of doing a synchronous local lookup — the tradeoff is a network round trip (and a brief loading state) where there used to be none. Adding a topic today is a content change — a new `writing_practice/<name>/overview.yaml` + `context.md` in the content repo, then a reload — not a frontend code change, a migration, or a new endpoint.
...
**Note:** `draft_object_key` gates `topic_id` with `_is_valid_topic_id` (alnum + hyphen) before building the S3 key — a path-safety check (no `/` or `..` reaching the object key), not a catalog lookup. `topic_id` values are minted by the grammar-content pipeline (decision 1's `overview.yaml` files, generated in the separate content repo), so whether a given id is a *real, current* topic is that pipeline's responsibility, not this app's — the same reasoning decision 8 uses for not validating `topic` against a catalog either.

### 7. Completion archives the text; `archive` entries are `{timestamp, content}`

`complete_draft` (`backend/utils/writing/writing_drafts.py`) is `save_draft` with one difference: instead of overwriting `archive`, it appends `{"timestamp": <ISO 8601 UTC>, "content": <text>}` to it and sets `draft` to that same text. It's called through `POST /writing-practice/<topic_id>/complete` from `WritingPracticeDetailPage.tsx`'s `settleReview` — the single frontend function that decides "is this text fully correct" (`checks.every(sentence => severity === "none")`, computed once) once every per-sentence check has settled, whether that's right after Submit or after the last mistake is fixed through the correction modal. `settleReview` is also what gates calling `record-usage` (decision 4) and the `writing_progress` `DONE` write below, so all three consequences of "the text is now fully correct" read the same one boolean instead of each re-deriving it.
...
**Postgres status.** `writing_practice.py`'s `_mark_progress` helper upserts a `writing_progress` row (`user_id`, `writing_topic` FK → `writing_practice.id`, `status`): `GET /writing-practice/<topic_id>` sets it to `WIP` on every open (including a re-open of an already-`DONE` topic — opening it to write something new is treated the same as starting), and `POST .../complete` sets it to `DONE`. Neither route re-checks correctness itself — both simply trust that the frontend only calls them when `settleReview` decided the text is fully correct, same as decision 4's usage recording. `GET /writing-practices` reads `status` back per topic (decision 1), defaulting to `TODO` for a topic never opened.
...
```text
loadGrammarData (dispatched once at login, App.tsx)
  │               GET /writing-practices ──► { writing_practices } (Postgres)
  │                                           writing_practices[].status ◄─ writing_progress
  ▼
GrammarPage (Grammar tab) reads the `grammar` slice, already populated
...
WritingPracticeDetailPage
  ...
  │                                                      + writing_progress.status = WIP (upsert)
  ...
  │      └──────────────────────────────► POST /writing-practice/<topic_id>/complete
  │                                           ├─► S3 (archive += entry)
  │                                           └─► writing_progress.status = DONE (upsert)
  │
