...
`POST /listening-practices/<id>/complete` (`backend/routes/complete_listening_practice.py`) takes `{"score": 0-100}` — the frontend's own percentage-correct across the exercises above — and upserts `listening_progress.status`: `DONE` at `score >= PASSING_SCORE` (80, the same threshold and variable name as `complete_grammar_point.py`), `WIP` otherwise. It does not touch `vocabulary_score`/`grammar_score` (that's `refresh`'s job) and does not persist the raw score itself — there's no column for it on `listening_progress`, matching the narrower scope this endpoint was asked to cover. See the [grammar content ADR](../adr/grammar-content.md) for the full detail-page shape (shadowing, blurred transcript, STT reuse, and the exercises/completion flow).
