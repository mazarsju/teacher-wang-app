...
`grammar_points` / `grammar_prerequisites` — grammar rule catalog. `grammar_points.id` is `"{hsk_level}|{title}"`; `s3_key` is the rule's folder key (e.g. `hsk1/01-basic-sentence-structure`) in the `GRAMMAR_CONTENT_S3_BUCKET` bucket (see [teacher-wang-grammar](https://github.com/mazarsju/teacher-wang-grammar)). `grammar_prerequisites` is a `(grammar_id, prerequisite_id)` association table, both FK → `grammar_points.id`. `POST /admin/grammar/reload` clears and repopulates both from every `grammar.yaml` in the bucket (`backend/utils/grammar/grammar_content_loader.py`); set `GRAMMAR_CONTENT_S3_PATH` to a local `teacher-wang-grammar`-layout checkout to reload from disk instead, for local debugging without AWS credentials. Decision notes: [grammar content architecture](../adr/grammar-content.md).
