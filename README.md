...
| `GET` | `/listening-practices` | List `listening_practice` topics with `hsk_level` at or below the caller's current HSK level + 1, each with the caller's `listening_progress` (`status`/`vocabulary_score`/`grammar_score`, defaulting to `TODO`/`0`/`0` for a topic never refreshed), plus a top-level `current_hsk_level` (the caller's own achieved HSK level, defaulting to 1) that `ListeningPage`'s HSK level/type/"Made for you" filters use |
