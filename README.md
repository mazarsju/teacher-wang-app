...
- [Voice interaction (TTS)](docs/adr/voice-interaction.md) — per-character OpenAI voices, HSK-derived speed + learner adjustment, reading-first vs. listening-first mode, every plan token-gated
...
This app does not use ElevenLabs — chat TTS is OpenAI-only (see [voice interaction](docs/adr/voice-interaction.md)). ElevenLabs is used only in a separate repository that pre-generates the listening-practice audio catalog.

#### Token budget

Every account gets a monthly token allowance (`settings.available_token`) — **100 000** on free (`FREE_PLAN_MAX_ALLOWED_TOKEN`), **10 000 000** on pro (`PRO_PLAN_TOKEN_GRANT`) — enforced on every LLM invoke, refilled at the start of each calendar month. Preferences shows remaining vs max; historical usage lives in `token_count`. Design notes: [plan management](docs/adr/plan-management.md).
...
| `GET` | `/token-usage` | Token history (`total_tokens`, `days`, …) plus `plan`, `available_token`, and `max_allowed_token` (100000 on free, 10000000 on pro) |
...
| `PATCH` | `/preferences/chat-setup` | Set either or both fields (partial body allowed). `listen_speed_adjustment` is added, as a fraction (e.g. `10` → `+0.10`), to the HSK-derived speed used by `/chat/tts` |
...
| `POST` | `/chat/tts` | Given `{ "text": "...", "voice": "..." }` (Chinese text; the frontend sends the speaking chat character's one fixed OpenAI voice, see `frontend/src/data/chatCharacters.ts`/`challenges.ts`), return an `audio/mpeg` clip from OpenAI TTS (`tts-1`). Playback speed is not client-supplied — it's derived server-side from the caller's chat-speaking HSK level (0.75 at HSK1 up to 1.1 at HSK6+, see `get_chat_tts_speed` in `backend/utils/knowledgeBase/hsk_level.py`), then adjusted by the caller's `listen_speed_adjustment` chat setup preference |
...
| `GET` | `/hsk-level-light` | Returns `{ current_level }`: just the learner's persisted HSK level (`Setting` row, `get_stored_current_hsk_level`) — no character/vocabulary scan, unlike `GET /hsk-level`. Used to decide which HSK levels' grammar points to load |
| `GET` | `/grammar-points-light` | Returns `{ grammar_points }`: every grammar point's catalog-only fields (`id`, `hsk_level`, `index` — the numeric folder prefix in `s3_key` —, `title`), ordered by HSK level then `index`. No per-user data, so it's cheap to load up front and doesn't touch `user_grammar_progress` |
| `GET` | `/grammar-points/<hsk_level>` | Returns `{ grammar_points }`: one HSK level's grammar points with the current user's `id`, `prerequisites`, `status`, `score`, and `usage_count` from `user_grammar_progress`/`grammar_prerequisites` — the Grammar tab calls this once per visible level (instead of one call for every level) once `GET /hsk-level-light` says which levels are visible |
| `GET` | `/writing-practices` | Returns `{ writing_practices }`: every `writing_practice` row (`id`/`title`/`after_grammar_point`/`status`), the catalog the Grammar tab uses to insert "Practice: `<title>`" rows after their grammar point (Postgres only, no S3 content) |
...
| `GET` | `/listening-practices-light` | Returns `{ listening_practices }`: catalog-only fields (`id`, `title`, `hsk_level`, `type`, `topic`, `translated_topic`), ordered by HSK level then id. Optional `?max_hsk_level=<n>` limits to topics at or below that level (omit for every level) — the frontend always passes it, scoped to the level `GET /hsk-level-light` just returned, so it never loads a catalog entry for a level it's about to filter out. No per-user data, so it's cheap to load up front and doesn't touch `listening_progress` |
| `GET` | `/listening-practices/<hsk_level>` | Returns `{ listening_practices }`: one HSK level's topics with `id`, `status`, `vocabulary_score`, and `grammar_score`, recomputed fresh on every call and upserted into `listening_progress` (new rows start `TODO`; an existing row's `status` is left untouched). `vocabulary_score` is the percentage of the topic's `unique_chars` already in the caller's `character` rows; `grammar_score` is the percentage of its comma-separated `grammar_rules` already `DONE`/`MASTERED` in `user_grammar_progress`. The Listening tab calls this once per visible level (instead of one call refreshing every level) once `GET /hsk-level-light` says which levels are visible |
| `GET` | `/listening-practices/<topic_id>` | One topic's detail: Postgres metadata plus its full `text.txt` transcript, per-sentence `breakdown.json` (`[{id, mandarin, translation}]`, `translation` merged from `breakdown_<language>.json` for the caller's language, falling back to English), `exercises` (`exercises.json`/`exercises_<language>.json`'s `multiple_choice` entries — `[{id, type, question, choices, answer}]`, same shape as the grammar content pipeline's exercises), `bonus_question` (the same file's trailing `open_question` entry's `question` text, split out for the "Bonus: Writing practice" section, or `null` if none), `progress` (the learner's saved `exercises`/`shadowing`/`bonus` answers, parsed from `listening_progress.progress`, or `null` if never saved — see `POST .../progress` below), and `segment_ids` (sentence ids that have their own `audio/audio-<n>.mp3` shadowing clip) — `404` if the topic doesn't exist |
...
| `POST` | `/listening-practices/<topic_id>/complete` | Given `{ "completed": true|false }` — the learner's own yes/no call, not a score threshold — upserts `listening_progress.status` to `DONE`/`WIP`; existing `vocabulary_score`/`grammar_score` are left untouched — `404` if the topic doesn't exist |
| `POST` | `/listening-practices/<topic_id>/progress` | Given `{ "progress": {...} }` (the learner's `exercises`/`shadowing`/`bonus` answers, replaced wholesale — not merged), JSON-stringifies and upserts it into `listening_progress.progress`; returned as-is by `GET /listening-practices/<topic_id>`'s own `progress` field on the next load — `404` if the topic doesn't exist, `400` if `progress` isn't an object |
...
Weekly articles (`GET /weekly-articles`) follow a lazier pattern instead: Home fetches it once on first visit and caches the result in its own slice (`weeklyArticle`), so navigating away and back does not refetch.

Grammar points and listening practices both load in stages, but eagerly, dispatched once at login rather than from `GrammarPage`/`ListeningPage` themselves, so the data is already loading (or loaded) by the time the learner opens either tab. `loadGrammarData.ts` and `loadListeningData.ts` are both dispatched first, *before* `syncAppData` (`App.tsx`'s post-auth effect), so `GET /hsk-level-light` is the very first request of the login sequence, ahead of `syncAppData`'s own `characters`/`words`/`hsk-level`/`hsk-characters` calls and `GET /auth/me`. Each thunk fetches `GET /hsk-level-light` *itself* rather than sharing one fetch — an earlier version deduped this into a single shared thunk chained via `.then()`, but that raced under React StrictMode's double-invoked login effect (a skipped, condition-guarded re-dispatch of the shared thunk resolved immediately rather than waiting for the real in-flight fetch, so the chained code ran with the level still `null` and loaded only level 1) — so the two-call duplication is deliberate; each thunk's own `condition` guard only has to reason about its own state, with no cross-thunk dependency to race. `hsk.currentLevelLight` is set from whichever call resolves and never refetched (a manual **Synchro** re-runs `syncAppData` but not either of these). `loadGrammarData` fetches `GET /grammar-points-light` (the catalog), then one `GET /grammar-points/<hsk_level>` call per visible HSK level (1 up to the learner's achieved level + 1) plus `GET /writing-practices`, each upserting into the `grammar` slice (`items`, `writingPractices`, `loadedLevels`) as it resolves — an earlier level's data is never overwritten by a later one. `loadListeningData` mirrors this exactly for `GET /listening-practices-light?max_hsk_level=<target>` (the catalog is itself scoped to the same visible-level ceiling, unlike the grammar catalog which always returns every level) and one `GET /listening-practices/<hsk_level>` call per visible level, upserting into the `listening` slice (`items`, `loadedLevels`). `GrammarPage`/`ListeningPage` only read this state (showing a spinner in place of the status/score — or, on the Listening tab, the overall-score icon and tile border, in neutral grey — for any level not yet in `loadedLevels`) and never fetch it themselves. Both thunks run at most once per session. (Home's own `GET /weekly-articles` fetch, above, is an unrelated, independent effect on `HomePage` mount and can still land before `hsk-level-light` if Home is the first page shown — the login-sequence ordering guarantee is scoped to `syncAppData`/`loadGrammarData`/`loadListeningData`/`auth/me`, not to every fetch any page might fire on its own.)

Both still clear on logout along with the rest of the store.
...
- [x] Listening challenges
  - [x] Listening-practice catalog and per-user progress ("Listening and Speaking" nav section, a mosaic ordered by overall score with a per-lesson fit dialog; `GET /listening-practices-light` for the catalog, one `GET /listening-practices/<hsk_level>` call per visible level computing `vocabulary_score`/`grammar_score`; admin S3 reload — see [listening practice](docs/adr/listening-practice.md), [schema tenancy](docs/architecture/schema-tenancy.md))
  ...
  - [x] Bonus writing question: `exercises.json`'s trailing `open_question` entry renders as a "Bonus: Writing practice" section after Full text (Completion comes last), reusing writing practice's own AI checks (`POST /writing/check-topic-relevance`, `POST /writing/check-sentence`) for a free-text answer — see [listening practice](docs/adr/listening-practice.md) decision 3b
  - [x] Answer persistence: a single `listening_progress.progress` JSON blob (exercises/shadowing/bonus answers) saved wholesale on every Verify/Check/Submit click and restored when the topic is reopened — see [listening practice](docs/adr/listening-practice.md) decision 7
