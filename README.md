...
- [x] Listening challenges
  ...
  - [x] Bonus writing question: `exercises.json`'s trailing `open_question` entry renders as a "Bonus: Writing practice" section at the bottom of the detail page, reusing writing practice's own AI checks (`POST /writing/check-topic-relevance`, `POST /writing/check-sentence`) for a free-text answer — see [listening practice](docs/adr/listening-practice.md) decision 3b
