...
def run_weekly_article_generation() -> dict | None:
    """Fetch China news and persist this week's per-HSK-level reading set.

    Returns the pipeline summary, or ``None`` when the news API returned
    nothing. Raises ``ValueError`` for missing keys / fetch failures.
    """
